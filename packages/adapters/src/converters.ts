...
export class MarkdownConverter implements DocumentConverter {
  ...
  async convert(
    ...
    const output = join(
      outputDir,
      `${basename(artifact.filename, extname(artifact.filename)) || "source"}.md`,
    );
    if (artifact.localPath !== output)
      await copyFile(artifact.localPath, output);
    return {
      markdownPath: output,
      title: basename(artifact.filename, extname(artifact.filename)).replace(
        /[-_]+/g,
        " ",
      ),
      adapter: this.name,
  ...
  async convert(
    ...
    try {
      const response = await fetch(
        `${this.options.endpoint.replace(/\/$/, "")}/convert`,
        {
          method: "POST",
      ...
      if (!response.ok)
        throw new Error(
          `MarkItDown service failed: ${response.status} ${await response.text()}`,
        );
      const result = (await response.json()) as Record<string, unknown>;
      if (typeof result.markdown_path !== "string")
        throw new Error("MarkItDown service response missing markdown_path");
      return {
        markdownPath: result.markdown_path,
        title:
          typeof result.title === "string"
            ? result.title
            : basename(artifact.filename, extname(artifact.filename)),
        adapter: this.name,
        adapterVersion:
