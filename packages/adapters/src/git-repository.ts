...
export class GitRepositoryProvider implements RepositoryProvider {
  async open(target: RepositoryTarget): Promise<RepositorySession> {
    ...
    await mkdir(join(target.workspaceRoot, "repositories"), {
      recursive: true,
    });
    ...
    if (!(await exists(join(controlRoot, ".git")))) {
      ...
      await git(target.workspaceRoot, [
        "clone",
        "--no-tags",
        target.remote,
        controlRoot,
...
class LocalWorkingRepositorySession implements RepositorySession {
  ...
  async begin(operationId: string): Promise<RepositoryOperation> {
    const operationRoot = join(
      this.target.workspaceRoot,
      "operations",
      operationId,
    );
    await mkdir(join(this.target.workspaceRoot, "operations"), {
      recursive: true,
    });
    await rm(operationRoot, { recursive: true, force: true });
    const currentBranch = (
      await git(this.repositoryRoot, ["branch", "--show-current"])
    ).trim();
    ...
    const baseSha = (
      await git(this.repositoryRoot, ["rev-parse", "HEAD"])
    ).trim();
...
class LocalWorkingGitOperation implements RepositoryOperation {
  ...
  async commit(message: string): Promise<string | null> {
    ...
    if (current !== this.baseSha) {
      throw new Error(
        `Brain branch moved during operation ${this.operationId}; refusing non-atomic update`,
      );
    }
    ensureClean(await git(this.repositoryRoot, ["status", "--porcelain"]));
  ...
  async push(): Promise<void> {
    const remotes = (await git(this.repositoryRoot, ["remote"]))
      .trim()
      .split(/\s+/)
      .filter(Boolean);
    ...
    try {
      ...
      await git(this.repositoryRoot, ["reset", "--hard", this.baseSha]).catch(
        () => undefined,
      );
      throw error;
    }
...
class ExistingRemoteSession implements RepositorySession {
  ...
  async begin(operationId: string): Promise<RepositoryOperation> {
    ...
    await rm(operationRoot, { recursive: true, force: true });
    const baseRef = this.target.push
      ? `origin/${this.target.branch}`
      : this.target.branch;
    await git(this.controlRoot, [
      "worktree",
      "add",
      "--detach",
      operationRoot,
    ...
    return new GitOperation(
      ...
      async (sha) => {
        if (!this.target.push)
          await git(this.controlRoot, ["reset", "--hard", sha]);
      },
      async () => {
        await git(this.controlRoot, [
          "worktree",
...
async function ensureLocalRepositoryIfNeeded(
  target: RepositoryTarget,
): Promise<void> {
  ...
  try {
    ...
    await git(path, [
      ...
      "--allow-empty",
      "-m",
      "chore: initialize brain repository",
    ]);
  }
}
...
function ensureClean(output: string): void {
  if (output.trim())
    throw new Error(`Repository control checkout is not clean:\n${output}`);
}
function ensureInside(root: string, candidate: string): void {
  ...
  if (b !== a && !b.startsWith(`${a}/`))
    throw new Error("Repository operation escaped workspace root");
}
