...
export class SafeSourceAcquirer implements SourceAcquirer {
  ...
  async acquire(request: AcquireRequest): Promise<AcquiredArtifact> {
    ...
    if (!/^https?:\/\//i.test(request.source))
      return this.acquireLocal(request);
  ...
  private async acquireRemote(
    ...
    for (
      let redirect = 0;
      redirect <= this.options.redirectLimit;
      redirect += 1
    ) {
      ...
      try {
        response = await fetch(url, {
          ...
          headers: {
            "user-agent":
              "Wikiplane/0.1 (+https://github.com/stefanrossmeier/wikiplane)",
          },
        });
      } finally {
        clearTimeout(timer);
      ...
      if ([301, 302, 303, 307, 308].includes(response.status)) {
        ...
        if (!location)
          throw new Error(
            `Redirect ${response.status} did not include Location`,
          );
        if (redirect === this.options.redirectLimit)
    ...
    if (!response)
      throw new Error("Acquisition failed before receiving a response");
    if (!response.ok)
      throw new Error(
        `HTTP acquisition failed: ${response.status} ${response.statusText}`,
      );
    ...
    if (
      ...
      throw new Error(
        `Remote source exceeds configured maximum of ${this.options.maxBytes} bytes`,
      );
    }
    ...
    if (mediaType && !isAllowedMediaType(mediaType)) {
      await response.body
        ?.cancel("Unsupported Wikiplane content type")
        .catch(() => undefined);
...
async function readLimitedBody(
  ...
  try {
    while (true) {
      ...
      if (total > maxBytes) {
        await reader
          .cancel("Wikiplane response size limit exceeded")
          .catch(() => undefined);
        throw new Error(
          `Remote source exceeds configured maximum of ${maxBytes} bytes`,
        );
      }
      chunks.push(value);
    }
    ...
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(
          () =>
            reject(
              new Error(`Remote source body timed out after ${timeoutMs} ms`),
            ),
          timeoutMs,
        );
      }),
...
export async function assertSafeRemoteUrl(url: URL): Promise<void> {
  ...
  for (const { address } of addresses) {
    if (isPrivateAddress(address))
      throw new Error(`Private/local network target is blocked: ${address}`);
  }
}
