...
const contradictionSchema = z.object({
  ...
  conflicting_source_ids: z.array(z.string().refine(isSourceId, 'invalid source id')).max(20).default([]),
});
...
export class KnowledgeCompiler {
  ...
  async integrate(input: {
    ...
    const candidateContext = await buildCandidateContext(input.brainRoot, input.markdown);
    const response = await atStage('MODEL_FAILED', () => this.models.complete({
      role: 'integrate',
    ...
    const parsed = integrationResponseSchema.parse(parseJsonObject(response.content));
    ...
    for (const rawMutation of parsed.knowledge) {
      ...
      const result = await upsertKnowledgePage(input.brainRoot, input.source, mutation);
    ...
    for (const rawContradiction of parsed.contradictions) {
      ...
      const result = await upsertContradiction(input.brainRoot, input.source, contradiction);
      if (result.changed && !changed.includes(result.path)) changed.push(result.path);
      if (result.changed && !created.includes(result.path) && !updated.includes(result.path)) updated.push(result.path);
    }

    const crosslinkResult = await atStage('CROSSLINK_FAILED', () => this.crosslink({
    ...
    return {
      ...
      modelUsage: [
        { role: 'integrate', model: response.model, provider: response.provider, usage: response.usage ?? {} },
        ...(crosslinkResult.model ? [{ role: 'crosslink', model: crosslinkResult.model, provider: crosslinkResult.provider, usage: crosslinkResult.usage ?? {} }] : []),
      ],
    };
  }
  ...
  private async crosslink(input: {
    brainRoot: string;
    changedPages: string[];
  }): Promise<{ added: number; usage?: Record<string, unknown>; model?: string; provider?: string }> {
    ...
    for (const fullPath of await listPages(wikiDir)) {
      ...
      if (!rel.startsWith('concepts/') && !rel.startsWith('entities/')) continue;
...
Return ONLY JSON {"links":[{"from":"concepts/a.md","to":"entities/b.md","reason":"..."}]}. The from path must be one of the changed pages.`,
    ...
    const parsed = crosslinkResponseSchema.parse(parseJsonObject(response.content));
    let added = 0;
    ...
    for (const link of parsed.links.slice(0, 50)) {
      ...
      if (link.from === link.to || !existingPaths.has(link.from) || !existingPaths.has(link.to)) continue;
    ...
    return { added, usage: response.usage, model: response.model, provider: response.provider };
  }
}
...
async function upsertKnowledgePage(
  ...
  const sourceIds = unique([...(asStringArray(existing?.frontmatter.sources) ?? []), source.id]);
  const tags = unique([...(asStringArray(existing?.frontmatter.tags) ?? []), ...mutation.tags]);
  const sourceLink = relative(join(brainRoot, 'wiki', dir), join(brainRoot, 'wiki', source.sourcePagePath)).replace(/\\/g, '/');
  const block = claimBlock(source, sourceLink, mutation.claims);
  const body = replaceSourceBlock(
    existing?.body ?? `# ${mutation.title}\n\n${mutation.summary ? `${mutation.summary}\n\n` : ''}## Claims`,
    `claims:${source.id}`,
    block,
  );
  ...
  const baseFrontmatter = {
    ...
    id: existing?.frontmatter.id ?? `${mutation.page_type}_${slugify(mutation.title)}`,
  ...
  const semanticallyChanged = !existing ||
    existing.body !== body ||
    JSON.stringify({ ...existing.frontmatter, updated: undefined }) !== JSON.stringify({ ...baseFrontmatter, updated: undefined });
...
async function upsertContradiction(
  ...
  const nextBody = replaceSourceBlock(page.body, `contradictions:${source.id}`, block, '## Contradictions');
  const nextSources = unique([...(asStringArray(page.frontmatter.sources) ?? []), ...related]);
  const changed = nextBody !== page.body || JSON.stringify(nextSources) !== JSON.stringify(asStringArray(page.frontmatter.sources) ?? []);
  if (changed) {
    page.body = nextBody;
...
function claimBlock(source: StoredSource, sourceLink: string, claims: string[]): string {
...
function replaceSourceBlock(body: string, key: string, block: string, sectionHeader = '## Claims'): string {
...
export function removeSourceBlocks(body: string, sourceId: string): string {
  ...
  next = next.replace(contradictionPattern, (block) => (block.includes(`\`${sourceId}\``) ? '\n' : block));
  return next.replace(/\n{3,}/g, '\n\n').trim();
}

async function buildCandidateContext(brainRoot: string, markdown: string): Promise<string> {
  const entries = await readIndex(join(brainRoot, 'wiki', 'index.md'));
  const terms = significantTerms(markdown);
  const scored = entries
    .filter((entry) => entry.category === 'Concepts' || entry.category === 'Entities')
    .map((entry) => ({
      entry,
      score: terms.reduce(
        (score, term) => score + occurrences(`${entry.title} ${entry.summary} ${entry.tags.join(' ')}`, term),
        0,
      ),
    }))
...
function normalizeKnowledgeMutation(
  ...
  if (!title) throw new Error('Model knowledge title became empty after sanitization');
  return {
    ...mutation,
    title,
    summary: plainMetadata(mutation.summary, 1000),
    tags: unique(mutation.tags.map((tag) => slugify(tag)).filter(Boolean)).slice(0, 20),
    claims: mutation.claims.map((claim) => safeMarkdownSentence(claim, 2000)).filter(Boolean),
  };
}
...
function normalizeContradiction(
  ...
  if (!title) throw new Error('Model contradiction title became empty after sanitization');
  if (!statement) throw new Error('Model contradiction statement became empty after sanitization');
  return {
    ...contradiction,
    title,
    statement,
...
function integrationSystemPrompt(sourceId: string, prompts: WikiplanePrompts): string {
...
const STOP_WORDS = new Set(['that', 'with', 'from', 'this', 'have', 'will', 'into', 'their', 'about', 'there', 'which', 'when', 'were', 'been', 'also', 'than', 'then', 'they', 'them', 'your', 'using']);
