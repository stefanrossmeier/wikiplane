...
const configSchema = z.object({
  ...
  adapters: z.object({
    ...
    ocr: z.enum(['markitdown-ocr', 'none']).default('markitdown-ocr'),
  }),
  models: z.object({
  ...
  ingest: z.object({
    ...
    max_download_bytes: z.number().int().positive().default(50 * 1024 * 1024),
...
export function parseConfig(input: unknown, baseDir = process.cwd()): WikiplaneConfig {
  ...
  return {
    ...
    brain: {
      ...
      remote: isPathLike(parsed.brain.remote) ? resolvePath(baseDir, parsed.brain.remote) : parsed.brain.remote,
    },
  };
}
