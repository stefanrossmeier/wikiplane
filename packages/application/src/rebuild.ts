...
export async function rebuildBrain(
  ...
  try {
    ...
    schemaFindings.push({
      severity: "error",
      category: "schema",
      message: "Brain repository is missing AGENTS.md.",
      file: "AGENTS.md",
    });
  }
  ...
  for (const entry of entries)
    backlinkCount += (await getBacklinks(wikiDir, entry.path)).length;
...
async function deriveIndexEntries(wikiDir: string): Promise<IndexEntry[]> {
  const pages = (await listPages(wikiDir)).filter((path) => {
    ...
    return (
      rel !== "index.md" && rel !== "log.md" && !rel.startsWith("queries/")
    );
  });
...
async function validateSchema(
  ...
  for (const path of pages) {
    const rel = relative(wikiDir, path).replace(/\\/g, "/");
    if (rel === "index.md" || rel === "log.md" || rel.startsWith("queries/"))
      continue;
    const page = await readPage(path);
    if (
      page.frontmatter.type === "source" &&
      typeof page.frontmatter.id === "string" &&
      isSourceId(page.frontmatter.id)
    ) {
  ...
  for (const path of pages) {
    ...
    if (!["source", "concept", "entity"].includes(String(type ?? ""))) {
      findings.push({
        ...
        message: "Missing or invalid page type.",
        file: `wiki/${rel}`,
      });
    }
    for (const key of ["id", "title"] as const) {
      if (
        typeof page.frontmatter[key] !== "string" ||
        !String(page.frontmatter[key]).trim()
      ) {
        findings.push({
          severity: "error",
          category: "frontmatter",
    ...
    if (typeof page.frontmatter.id === "string") {
      ...
      if (duplicate) {
        findings.push({
          ...
          message: `Duplicate id ${page.frontmatter.id} also used by ${duplicate}`,
          file: `wiki/${rel}`,
        });
    ...
    if (type === "source") {
      if (
        ...
        findings.push({
          severity: "error",
          category: "source-id",
          message: "Source page has an invalid source id.",
          file: `wiki/${rel}`,
        });
        ...
        findings.push({
          severity: "error",
          category: "source-id",
          message: `Source page filename must match id ${page.frontmatter.id}.`,
          file: `wiki/${rel}`,
        });
      }
    }
    if (type === "concept" || type === "entity") {
      if (!Array.isArray(page.frontmatter.sources)) {
        findings.push({
          ...
          message: "Knowledge page frontmatter sources must be an array.",
          file: `wiki/${rel}`,
        });
      } else {
        for (const sourceId of page.frontmatter.sources.filter(
          (value): value is string => typeof value === "string",
        )) {
          if (!isSourceId(sourceId) || !sourceIds.has(sourceId)) {
            findings.push({
              severity: "error",
              category: "missing-sources",
              message: `Knowledge page references missing or invalid source id: ${sourceId}`,
              file: `wiki/${rel}`,
            });
          }
        }
    ...
    if (type === "source") {
      if (
        ...
        !page.frontmatter.source_url
      ) {
        findings.push({
          severity: "error",
          category: "provenance",
          message: "Source page is missing exact source_url provenance.",
          file: `wiki/${rel}`,
        });
      }
      if (
        typeof page.frontmatter.source_path !== "string" ||
        !page.frontmatter.source_path.startsWith("raw/")
      ) {
        findings.push({
          severity: "error",
          category: "provenance",
          message: "Source page is missing raw source_path.",
          file: `wiki/${rel}`,
        });
      } else {
        ...
        if (
          ...
          resolve(rawRoot, withinRaw) !== rawCandidate
        ) {
          findings.push({
            severity: "error",
            category: "provenance",
            message: `Source transcription path escapes raw/: ${page.frontmatter.source_path}`,
            file: `wiki/${rel}`,
          });
        } else {
          try {
            ...
            findings.push({
              severity: "error",
              category: "provenance",
              message: `Source transcription is missing: ${page.frontmatter.source_path}`,
              file: `wiki/${rel}`,
            });
          }
        }
