...
import { findSourceByReference, findSourceById, storeConvertedSource } from './source-store.js';
...
export interface IngestResult {
  ...
  validation: { broken_links: number; missing_sources: number; errors: number; warnings: number };
  commit: string | null;
}
...
export class WikiplaneService {
  ...
  constructor(
    ...
    this.compiler = new KnowledgeCompiler(deps.models, deps.prompts ?? DEFAULT_PROMPTS);
  }

  async brainInit(): Promise<{ status: 'success'; operation_id: string; commit: string | null }> {
    return this.transaction('chore(knowledge): initialize wikiplane brain', async (root, op) => {
      await initializeBrain(root, this.deps.agentsTemplate ?? DEFAULT_AGENTS_TEMPLATE);
      const validation = await rebuildBrain(root);
      assertValid(validation.errors, 'Brain initialization validation failed');
  ...
  private async ingestInternal(sourceReference: string, expectedSourceId?: string): Promise<IngestResult> {
    const op = operationId();
    const telemetry = new OperationTelemetry(this.config.brain.workspace_root, op);
    await telemetry.event('operation_started', { source_reference: sourceReference });
    ...
    try {
      await initializeBrain(operation.root, this.deps.agentsTemplate ?? DEFAULT_AGENTS_TEMPLATE);
      acquired = await telemetry.stage('acquire', () => atStage('ACQUIRE_FAILED', () => this.deps.acquirer.acquire({
        source: sourceReference,
        operationId: op,
        workspaceDir: artifacts,
      })));
      await telemetry.event('artifact_acquired', { media_type: acquired.mediaType, bytes: acquired.byteLength, resolved_reference: acquired.resolvedReference });
      const converted = await telemetry.stage('convert', () => atStage('CONVERT_FAILED', () => this.convert(acquired!, artifacts)));
      await telemetry.event('source_converted', { adapter: converted.adapter, adapter_version: converted.adapterVersion, diagnostics: sanitizeDiagnostics(converted.diagnostics) });
      ...
      const existing = expectedSourceId
        ...
        : await findSourceByReference(operation.root, acquired.suppliedReference);
      if (expectedSourceId && !existing) throw new Error(`Source not found during refresh: ${expectedSourceId}`);
      const source = await storeConvertedSource({
        brainRoot: operation.root,
        suppliedReference: existing?.suppliedReference ?? acquired.suppliedReference,
        resolvedReference: acquired.resolvedReference ?? existing?.resolvedReference,
        converted,
        markdown,
        existing,
      });
      const compile = source.changed || !existing
        ? await telemetry.stage('integrate', () => atStage('INTEGRATION_FAILED', () => this.compiler.integrate({ brainRoot: operation.root, source, markdown })))
      ...
      const validation = await telemetry.stage('rebuild', () => atStage('REBUILD_FAILED', () => rebuildBrain(operation.root)));
      assertValid(validation.errors, 'Ingest validation failed');
      const meaningfulChange = Boolean(source.changed || compile.created.length || compile.updated.length || compile.crossLinksAdded || validation.changed);
      if (meaningfulChange) {
      ...
      const commit = await telemetry.stage('commit', () => atStage('GIT_FAILED', () => operation.commit(`feat(knowledge): ingest ${source.title}`)));
      if (commit && this.config.git.push) await telemetry.stage('push', () => atStage('PUSH_FAILED', () => operation.push()));
      await telemetry.event('operation_completed', { source_id: source.id, commit, validation_errors: validation.errors, validation_warnings: validation.warnings });
      const createdTypes = await countPageTypes(operation.root, compile.created);
      return {
        status: 'success',
        operation_id: op,
        source_id: source.id,
      ...
      await telemetry.event('operation_failed', {
        failure_stage: error && typeof error === 'object' && 'stage' in error ? String((error as { stage: unknown }).stage) : 'UNKNOWN',
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
    } finally {
      await acquired?.cleanup?.().catch(() => undefined);
      await rm(artifacts, { recursive: true, force: true }).catch(() => undefined);
  ...
  async removeSource(sourceId: string): Promise<unknown> {
    return this.transaction(`feat(knowledge): remove ${sourceId}`, async (root, op) => {
      ...
      return { status: 'success', operation_id: op, ...result, validation: validationSummary(validation) };
    });
  }

  async rebuild(check = false): Promise<unknown> {
    if (check) {
      return this.readOnly(async (root) => ({ status: 'success', ...(await rebuildBrain(root, true)) }));
    }
    return this.transaction('chore(knowledge): rebuild derived state', async (root, op) => {
  ...
  async lint(): Promise<unknown> {
    return this.readOnly(async (root) => ({ status: 'success', ...(await lintBrain(root)) }));
  }
  ...
  async query(question: string): Promise<unknown> {
    return this.readOnly((root) => queryBrain(root, question, this.deps.models, this.deps.prompts?.query ?? DEFAULT_PROMPTS.query));
  }

  async recompile(): Promise<unknown> {
    return this.transaction('feat(knowledge): recompile wiki', async (root, op) => {
      ...
      const rawFiles = (await readdir(join(root, 'raw'))).filter((name) => name.endsWith('.md')).sort();
      ...
      for (const name of rawFiles) {
        ...
        if (!source) throw new Error(`Raw transcription ${name} has no source page ${id}`);
        await this.compiler.integrate({ brainRoot: root, source, markdown: raw.body });
      ...
      return { status: 'success', operation_id: op, sources_compiled: sourcesCompiled, validation: validationSummary(validation) };
    });
  }
  ...
  private async convert(
    ...
    if (artifact.mediaType?.includes('html') || /\.html?$/i.test(artifact.filename)) {
      return this.deps.webConverter.convert(artifact, outputDir);
    }
    const converter = this.deps.documentConverters.find((candidate) => candidate.supports(artifact));
    if (!converter) throw new Error(`No DocumentConverter supports ${artifact.filename} (${artifact.mediaType ?? 'unknown'})`);
  ...
  private async readOnly<T>(fn: (root: string) => Promise<T>): Promise<T> {
    ...
    const op = operationId();
    const operation = await session.begin(op);
    try { return await fn(operation.root); }
    finally { await operation.close().catch(() => undefined); await session.close().catch(() => undefined); }
  }
...
function validationSummary(result: { errors: number; warnings: number; findings: Array<{ category: string }> }) {
  return {
    broken_links: result.findings.filter((finding) => finding.category === 'broken-links').length,
    missing_sources: result.findings.filter((finding) => finding.category === 'provenance' || finding.category === 'missing-sources').length,
    errors: result.errors,
    warnings: result.warnings,
  };
}
async function countPageTypes(root: string, paths: string[]): Promise<{ concepts: number; entities: number }> {
  let concepts = 0;
  let entities = 0;
...
function sanitizeDiagnostics(value: Record<string, unknown> | undefined): Record<string, unknown> | undefined {
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    else if (Array.isArray(item)) output[key] = item.slice(0, 50).map((entry) =>
      typeof entry === 'string' ? entry.slice(0, 200) : typeof entry === 'number' || typeof entry === 'boolean' || entry === null ? entry : '[omitted]',
    );
  }
  return output;
}
