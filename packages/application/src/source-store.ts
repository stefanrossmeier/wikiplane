...
export async function findSourceByReference(
  ...
  for (const name of names.filter((value) => value.endsWith('.md'))) {
    ...
    assertSourceId(id);
    if (name !== `${id}.md`) throw new Error(`Source page filename/id mismatch: ${name} contains ${id}`);
    const rawPath = safeRawPath(brainRoot, String(page.frontmatter.source_path ?? ''));
    return {
      id,
      title: String(page.frontmatter.title ?? id),
      suppliedReference,
      resolvedReference:
        typeof page.frontmatter.resolved_url === 'string' ? page.frontmatter.resolved_url : undefined,
      rawPath,
...
export async function storeConvertedSource(input: {
  ...
  assertSourceId(id);
  const title = safeTitle(input.converted.title?.trim() || input.existing?.title || titleFromPath(input.converted.markdownPath));
  const rawName = `${id}-${slugify(title) || 'source'}.md`;
  const rawRel = safeRawPath(input.brainRoot, input.existing?.rawPath || `raw/${rawName}`);
  ...
  const metadataChanged =
    !existingRaw ||
    String(existingRaw.frontmatter.source_url ?? '') !== input.suppliedReference ||
    nullableString(existingRaw.frontmatter.resolved_url) !== (input.resolvedReference ?? undefined) ||
    String(existingRaw.frontmatter.adapter ?? '') !== input.converted.adapter ||
    nullableString(existingRaw.frontmatter.adapter_version) !== input.converted.adapterVersion ||
    String(existingRaw.frontmatter.title ?? '') !== title;
  const transcriptionChanged = !existingRaw || existingRaw.body !== normalizedMarkdown;
  ...
  const retrievedAt = changed
    ...
    : input.existing?.retrievedAt || String(existingRaw?.frontmatter.retrieved_at ?? new Date().toISOString());
  ...
  if (changed) {
    ...
    const rawLink = relative(join(input.brainRoot, 'wiki', 'sources'), rawFull).replace(/\\/g, '/');
    await writePage(sourceFull, {
      ...
      body: [
        ...
        input.resolvedReference && input.resolvedReference !== input.suppliedReference
...
function escapeMarkdownText(value: string): string {
  return value.replace(/([\\[\]`])/g, '\\$1').replaceAll('<', '&lt;').replaceAll('>', '&gt;');
}
...
function safeRawPath(brainRoot: string, value: string): string {
  ...
  if (!withinRaw || withinRaw.startsWith('..') || resolve(rawRoot, withinRaw) !== candidate) {
    throw new Error(`Raw source path escapes raw/: ${value}`);
  }
  return normalized;
}

export async function findSourceById(brainRoot: string, id: string): Promise<StoredSource | null> {
  assertSourceId(id);
  ...
  if (storedId !== id) throw new Error(`Source page ${id}.md contains mismatched id ${storedId}`);
  const suppliedReference = String(page.frontmatter.source_url ?? '');
  if (!suppliedReference) throw new Error(`Source page ${id}.md is missing source_url`);
  return {
    id,
    title: String(page.frontmatter.title ?? id),
    suppliedReference,
