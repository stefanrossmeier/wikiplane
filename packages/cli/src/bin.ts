...
async function main(): Promise<void> {
  ...
  let result: unknown;
  if (command === 'brain' && subcommand === 'init') result = await service.brainInit();
  else if (command === 'ingest' && subcommand) result = await service.ingest(subcommand);
  else if (command === 'source' && subcommand === 'refresh' && rest[0]) result = await service.refresh(rest[0]);
  else if (command === 'source' && subcommand === 'remove' && rest[0]) result = await service.removeSource(rest[0]);
  else if (command === 'status') result = await service.status();
  else if (command === 'lint') result = await service.lint();
  else if (command === 'rebuild') result = await service.rebuild(args.includes('--check'));
  else if (command === 'recompile') result = await service.recompile();
  else if (command === 'query') result = await service.query([subcommand, ...rest].filter(Boolean).join(' '));
  else {
...
function isFailedCheck(value: unknown): boolean {
  return Boolean(value && typeof value === 'object' && 'errors' in value && Number((value as { errors: unknown }).errors) > 0);
}
