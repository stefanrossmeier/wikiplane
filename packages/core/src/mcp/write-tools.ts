    ...
    inputSchema: {
      ...
      properties: {
        pagePath: {
          type: "string",
          description:
            'Relative path within the wiki/ directory (e.g. "concepts/ai.md")',
        },
        title: {
          type: "string",
        ...
        type: {
          type: "string",
          description:
            'Page type, e.g. "entity", "concept", "source" (required)',
        },
        tags: {
          type: "array",
          items: { type: "string" },
    ...
    inputSchema: {
      ...
      properties: {
        name: {
          type: "string",
          description:
            "Concept name (used as title and slugified for filename)",
        },
        content: {
          type: "string",
    ...
    inputSchema: {
      ...
      properties: {
        pagePath: {
          type: "string",
          description:
            'Relative path of the source page within wiki/ (e.g. "concepts/ai.md")',
        },
        targetPages: {
          type: "array",
          items: { type: "string" },
          description:
            'Array of relative paths to link to (e.g. ["entities/openai.md"])',
        },
      },
    ...
    inputSchema: {
      ...
      properties: {
        pagePath: {
          type: "string",
          description:
            'Path of the index entry to update (e.g. "concepts/ai.md")',
        },
        summary: {
          type: "string",
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        force: {
          type: "boolean",
          description: "If true, re-ingest even if source was already ingested",
        },
      },
      required: ["sourcePath"],
      additionalProperties: false,
...
export async function handleWriteToolCall(
  ...
  switch (name) {
    ...
    case "wiki_create_concept": {
      ...
      const result = await createConceptPage(
        wikiDir,
        conceptName,
        content,
        tags,
    ...
    case "wiki_update_page": {
      ...
      if (newTitle !== undefined)
        validateLength(newTitle, "title", MAX_TITLE_LENGTH);
      ...
      if (bodyAppend !== undefined)
        validateLength(bodyAppend, "bodyAppend", MAX_BODY_LENGTH);
      const bodyReplace = optionalString(args, "bodyReplace");
      if (bodyReplace !== undefined)
        validateLength(bodyReplace, "bodyReplace", MAX_BODY_LENGTH);
    ...
    case "wiki_update_index": {
      ...
      if (summary !== undefined)
        validateLength(summary, "summary", MAX_BODY_LENGTH);
      const tags = optionalStringArray(args, "tags");
      validateTags(tags);
