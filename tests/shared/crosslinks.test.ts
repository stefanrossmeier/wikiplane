...
import {
  ...
  readPage,
  writePage,
} from "../../packages/core/src/wiki.js";
...
describe("addCrosslinks", () => {
  ...
  it("should throw listing all missing target pages", async () => {
    ...
    await expect(
      ...
      "Target pages not found: entities/missing1.md, entities/missing2.md",
    );
  });
  ...
  it("should avoid duplicate links in existing See also section", async () => {
    ...
    const page = await readPage(fromPath);
    const matches = page.body.match(
      /- \[OpenAI\]\(\.\.\/entities\/openai\.md\)/g,
    );
