...
describe("config", () => {
  ...
  it("rejects unknown concrete adapter selections before ingestion", () => {
    expect(() =>
      parseConfig({
        ...valid,
        adapters: { ...valid.adapters, web: "unconfigured-vendor" },
      }),
    ).toThrow();
  });
  ...
  it("rejects weakening exact provenance preservation", () => {
    expect(() =>
      parseConfig({
        ...valid,
        ingest: { ...valid.ingest, preserve_supplied_source_reference: false },
      }),
    ).toThrow();
  });
  ...
  it("rejects unsupported artifact/body retention settings instead of silently ignoring them", () => {
    expect(() =>
      parseConfig({
        ...valid,
        ingest: { ...valid.ingest, retain_original_artifact: true },
      }),
    ).toThrow();
    expect(() =>
      parseConfig({
        ...valid,
        logging: { ...valid.logging, retain_prompt_bodies: true },
      }),
    ).toThrow();
    expect(() =>
      parseConfig({
        ...valid,
        logging: { ...valid.logging, retain_document_bodies: true },
      }),
    ).toThrow();
  });
});
