...
import { WikiplaneService, type WikiplaneConfig } from '../../packages/application/src/index.js';
import { MarkdownConverter } from '../../packages/adapters/src/converters.js';
import { FileSourceAcquirer, ScriptedModelProvider, WorkingTreeRepositoryProvider } from '../../packages/testing/src/index.js';
...
function config(root: string): WikiplaneConfig {
  return {
    brain: { remote: root, branch: 'main', workspace_root: join(root, '.workspace') },
...
function models() {
  return new ScriptedModelProvider({
      ...
      knowledge: [
        {
          ...
          tags: ['git', 'markdown'],
          claims: ['Durable knowledge is stored in Markdown and versioned by Git.'],
        },
        {
          page_type: 'entity',
          title: 'Wikiplane',
          ...
          tags: ['wikiplane'],
          claims: ['Wikiplane compiles sources into a cross-referenced Markdown wiki.'],
        },
      ],
      contradictions: [],
    }),
    crosslink: JSON.stringify({
      links: [
        { from: 'concepts/git-native-knowledge.md', to: 'entities/wikiplane.md', reason: 'Wikiplane implements the concept.' },
      ],
    }),
    query: 'Wikiplane stores compiled knowledge in Git-tracked Markdown. [concepts/git-native-knowledge.md]',
  });
}
...
describe('end-to-end Markdown ingest', () => {
  it('preserves exact source identity, creates knowledge, crosslinks, and converges on re-ingest', async () => {
    ...
    const service = new WikiplaneService(config(root), {
      ...
      webConverter: { name: 'unused', async convert() { throw new Error('unused'); } },
      models: model,
    });
    ...
    const source = await readPage(join(root, 'wiki', 'sources', `${first.source_id}.md`));
    expect(source.frontmatter.source_url).toBe(input);
    const conceptPath = join(root, 'wiki', 'concepts', 'git-native-knowledge.md');
    const conceptBefore = await readFile(conceptPath, 'utf8');
    expect(conceptBefore).toContain(`wikiplane:claims:${first.source_id}:start`);
    ...
    const query = await service.query('How is durable knowledge stored?') as { answer: string; references: Array<{ path: string }> };
    expect(query.answer).toContain('Git-tracked Markdown');
    expect(query.references.some((ref) => ref.path === 'concepts/git-native-knowledge.md')).toBe(true);
  });
  ...
  it('preserves material contradictions and reevaluates them on source removal', async () => {
    ...
    await writeFile(inputA, '# Source A\n\nThe retention period is 90 days.', 'utf8');
    await writeFile(inputB, '# Source B\n\nThe retention period is 30 days.', 'utf8');
    let priorId = '';
      ...
      integrate: () => {
        ...
        if (integrationCall === 1) {
          return JSON.stringify({
            knowledge: [{ page_type: 'concept', title: 'Retention period', summary: 'Documented retention periods.', tags: [], claims: ['The retention period is 90 days.'] }],
            contradictions: [],
          });
        }
        return JSON.stringify({
          knowledge: [{ page_type: 'concept', title: 'Retention period', summary: 'Documented retention periods.', tags: [], claims: ['The retention period is 30 days.'] }],
          contradictions: [{ page_type: 'concept', title: 'Retention period', statement: 'This source says 30 days while the earlier source says 90 days.', conflicting_source_ids: [priorId] }],
        });
      },
