...
describe("brain trust boundary", () => {
    ...
    expect(
      result.findings.some(
        (finding) =>
          ...
          finding.message.includes("escapes raw/"),
      ),
    ).toBe(true);
  });
  ...
  it("rejects source page filename/id mismatches before using source metadata", async () => {
    ...
    await expect(findSourceById(root, "src_one")).rejects.toThrow(
      /mismatched id/i,
    );
  });
  ...
  it("sanitizes model-authored Markdown before deterministic filesystem writes", async () => {
    ...
    const result = await compiler.integrate({
      brainRoot: root,
      source,
      markdown: "content",
    });
  ...
  it("rejects model mutations whose sanitized title is empty", async () => {
    ...
    await expect(
      new KnowledgeCompiler(model).integrate({
        brainRoot: root,
        source,
        markdown: "content",
      }),
    ).rejects.toThrow(/title became empty/i);
  });
});
